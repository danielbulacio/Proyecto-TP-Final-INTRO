...
export async function getAllParcelas() {
    const res = await db.query(
        "SELECT nombre,latitud,longitud FROM parcelas"
    );
    return res.rows;
}
...
export async function getParcela(id) {
    const res = await db.query(
        "SELECT nombre,latitud,longitud FROM parcelas WHERE id = $1",
        [id]
    );
    return res.rows[0];
}
...
export async function updateParcela(id,nombre,latitud,longitud) {
    const res = await db.query(
        "UPDATE parcelas SET nombre=$1, latitud=$2, longitud=$3", [nombre, latitud, longitud]
    );
...
export async function createParcela(nombre,latitud,longitud){
    const res = await db.query(
        "INSERT INTO parcelas(nombres,latitud,longitud)VALUES ($1,$2,$3)",[nombre,latitud,longitud]
    );
    return res.rowCount == 1;
}